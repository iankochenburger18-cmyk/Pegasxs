...
export default function ExposeUserVideos() {
  ...
  React.useEffect(() => {
    function handleRenderStarted(event: Event) {
      ...
      if (!renderId) return
      setVideo(null); setMessage("Rendering..."); setActiveRenderId(renderId)
    }
  ...
  return (
    <div style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 24px 120px", boxSizing: "border-box" }}>
      <div style={{ width: "min(820px, 100%)", display: "grid", gap: 14, justifyItems: "center" }}>
        {message && (
          <div style={{ color: "white", fontSize: 16, fontWeight: 500, textAlign: "center" }}>{message}</div>
        )}
        {video?.signedUrl && (
          <>
            <video src={video.signedUrl} controls style={{ width: "100%", maxWidth: 820, borderRadius: 18, background: "black" }} />
            <button onClick={downloadVideo} disabled={downloading} style={{ padding: "12px 18px", background: "white", color: "black", border: "none", borderRadius: 8, fontWeight: 600, cursor: downloading ? "default" : "pointer", opacity: downloading ? 0.7 : 1 }}>
